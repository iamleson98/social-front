...
export const defaultAddressFormValues = () =>
	addressFieldsOrder.reduce(
		(prev, acc) => ({ ...prev, [acc]: { value: '', error: null } }),
		{} as Record<AddressField, AddressFieldObj>,
	);
...
export type AddressFieldValidator = (
	required: boolean,
	value?: string | number,
	countryCode?: CountryCode,
) => string | null;
...
export const addressFieldValidators: Record<AddressField, AddressFieldValidator> = {
	...
	countryCode: (required: boolean, value?: string | number) => {
		...
		if (
			...
			!(value.toUpperCase() in CountryCode)
		)
...
const getMissingFieldsFromAddress = (
	address: Maybe<AddressFragment>,
	validationRules: AddressValidationData,
) => {
	if (!address) return [];
