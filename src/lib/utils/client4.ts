import type {
	...
	BookingRoute,
	BookingSeat,
	BookingTransportBrand,
	BookingTrip,
	BookingVehicle,
	CancelBookingReservationRequest,
	FailBookingTripRequest,
...
import type {
	...
	OptsSignalExt,
	StatusOK,
} from '$lib/types/client4';
import type { SearchOpts } from '$lib/types/search';
import type { UserSearch } from '$lib/types/users';
import type {
	AccessControlPolicy,
	CELExpressionError,
	AccessControlTestResult,
	AccessControlPoliciesResult,
	AccessControlPolicyChannelsResult,
...
} from '@mattermost/types/remote_clusters';
...
export default class Client4 {
	...
	getBookingRoute() {
		return `${this.getBaseRoute()}/booking`;
	}

	getBookingAddressesRoute() {
		return `${this.getBookingRoute()}/addresses`;
	}

	getBookingAddressRoute(addressId: string) {
		return `${this.getBookingAddressesRoute()}/${addressId}`;
	}

	getBookingRoutesRoute() {
		return `${this.getBookingRoute()}/routes`;
	}

	getBookingRouteByIdRoute(routeId: string) {
		return `${this.getBookingRoutesRoute()}/${routeId}`;
	}

	getBookingTransportBrandsRoute() {
		return `${this.getBookingRoute()}/transport_brands`;
	}

	getBookingTransportBrandRoute(brandId: string) {
		return `${this.getBookingTransportBrandsRoute()}/${brandId}`;
	}

	getBookingVehiclesRoute() {
		return `${this.getBookingRoute()}/vehicles`;
	}

	getBookingVehicleRoute(vehicleId: string) {
		return `${this.getBookingVehiclesRoute()}/${vehicleId}`;
	}

	getBookingSeatsRoute() {
		return `${this.getBookingRoute()}/seats`;
	}

	getBookingSeatRoute(seatId: string) {
		return `${this.getBookingSeatsRoute()}/${seatId}`;
	}

	getBookingVehicleSeatsRoute(vehicleId: string) {
		return `${this.getBookingVehicleRoute(vehicleId)}/seats`;
	}

	getBookingTripsRoute() {
		return `${this.getBookingRoute()}/trips`;
	}

	getBookingTripRoute(tripId: string) {
		return `${this.getBookingTripsRoute()}/${tripId}`;
	}

	getBookingTripsByRouteRoute(routeId: string) {
		return `${this.getBookingRouteByIdRoute(routeId)}/trips`;
	}

	getBookingTripsByVehicleRoute(vehicleId: string) {
		return `${this.getBookingVehicleRoute(vehicleId)}/trips`;
	}

	getBookingReservationsRoute() {
		return `${this.getBookingRoute()}/reservations`;
	}

	getBookingReservationRoute(reservationId: string) {
		return `${this.getBookingReservationsRoute()}/${reservationId}`;
	}

	getBookingReservationsByTripRoute(tripId: string) {
		return `${this.getBookingTripRoute(tripId)}/reservations`;
	}

	getBookingReservationsByUserRoute(userId: string) {
		return `${this.getBookingRoute()}/users/${userId}/reservations`;
	}
	...
	resetFailedAttempts = (userId: string) => {
		return this.doFetch<StatusOK>(`${this.getUserRoute(userId)}/reset_failed_attempts`, {
			method: 'post',
		});
	};

	getKnownUsers = () => {
		return this.doFetch<Array<UserProfile['id']>>(`${this.getUsersRoute()}/known`, {
			method: 'get',
		});
	};
	...
	login = async (loginId: string, password: string, token = '', ldapOnly = false) => {
		...
		const { data: profile, headers } = await this.doFetchWithResponse<UserProfile>(
	...
	loginWithDesktopToken = async (token: string) => {
		...
		return this.doFetch<UserProfile>(`${this.getUsersRoute()}/login/desktop_token`, {
			method: 'post',
			body: JSON.stringify(body),
		});
	};
	...
	loginWithMagicLink = (token: string) => {
		...
		return this.doFetch<UserProfile>(`${this.getUsersRoute()}/login/one_time_link`, {
			method: 'post',
			body: JSON.stringify(body),
		});
	};
	...
	getUsersForReporting = (filter: UserReportOptions) => {
		...
		return this.doFetch<UserReport[]>(`${this.getReportsRoute()}/users${queryString}`, {
			method: 'get',
		});
	};
	...
	getUserCountForReporting = (filter: UserReportFilter) => {
		const queryString = buildQueryString(filter);
		return this.doFetch<number>(`${this.getReportsRoute()}/users/count${queryString}`, {
			method: 'get',
		});
	};
	...
	startUsersBatchExport = (filter: UserReportFilter) => {
		const queryString = buildQueryString(filter);
		return this.doFetch<StatusOK>(`${this.getReportsRoute()}/users/export${queryString}`, {
			method: 'post',
		});
	};
	...
	searchUsers = (options: UserSearch) => {
	...
	moveThread = (postId: string, channelId: string) => {
		...
		return this.doFetch<StatusOK>(url, {
			method: 'post',
	...
	getServerLimits = () => {
		return this.doFetchWithResponse<ServerLimits>(`${this.getServerLimitsRoute()}`, {
			method: 'get',
		});
	};
	...
	getTeam = (teamId: string, asContentReviewer = false, flaggedPostId?: string) => {
		...
		return this.doFetch<Team>(`${this.getTeamRoute(teamId)}${queryParams}`, { method: 'get' });
	};
	...
	createChannel = (
		channel: PartialExcept<Channel, 'name' | 'display_name' | 'type' | 'team_id'>,
	) => {
		return this.doFetch<ServerChannel>(`${this.getChannelsRoute()}`, {
			method: 'post',
	...
	createDirectChannel = (userIds: string[]) => {
		return this.doFetch<ServerChannel>(`${this.getChannelsRoute()}/direct`, {
			method: 'post',
			body: JSON.stringify(userIds),
		});
	};
	...
	createGroupChannel = (userIds: string[]) => {
		return this.doFetch<ServerChannel>(`${this.getChannelsRoute()}/group`, {
			method: 'post',
			body: JSON.stringify(userIds),
		});
	};
	...
	unarchiveChannel = (channelId: string) => {
		return this.doFetch<ServerChannel>(`${this.getChannelRoute(channelId)}/restore`, {
			method: 'post',
		});
	};

	updateChannel = (channel: Channel) => {
		return this.doFetch<ServerChannel>(`${this.getChannelRoute(channel.id)}`, {
			method: 'put',
			body: JSON.stringify(channel),
		});
	};
	...
	getChannel = (channelId: string, asContentReviewer = false, flaggedPostId?: string) => {
		...
		const queryParams = buildQueryString(queryParamsArgs);
		return this.doFetch<ServerChannel>(`${this.getChannelRoute(channelId)}${queryParams}`, {
			method: 'get',
		});
	};
	...
	getChannelStats = (channelId: string, includeFileCount = false) => {
		...
		return this.doFetch<ChannelStats>(`${this.getChannelRoute(channelId)}/stats${param}`, {
			method: 'get',
		});
	};

	getChannelsMemberCount = (channelIds: string[]) => {
		return this.doFetch<Record<string, number>>(`${this.getChannelsRoute()}/stats/member_count`, {
			method: 'post',
	...
	readMultipleChannels = (channelIds: string[]) => {
		return this.doFetch<ChannelViewResponse>(`${this.getChannelsRoute()}/members/me/mark_read`, {
			method: 'post',
			body: JSON.stringify(channelIds),
		});
	};
	...
	searchAllChannels(
		term: string,
		opts: { page: number; per_page: number } & ChannelSearchOpts & OptsSignalExt,
	): Promise<ChannelsWithTotalCount>;
	searchAllChannels(
		term: string,
		opts: (Omit<ChannelSearchOpts, 'page' | 'per_page'> & OptsSignalExt) | undefined,
	): Promise<ChannelWithTeamData[]>;
	searchAllChannels(term: string, opts: ChannelSearchOpts & OptsSignalExt = {}) {
		...
		} = { include_deleted: includeDeleted, exclude_remote: excludeRemote };
		if (nonAdminSearch) {
	...
	createChannelBookmark = (
		channelId: string,
		channelBookmark: ChannelBookmarkCreate,
		connectionId: string,
	) => {
		return this.doFetch<ChannelBookmark>(`${this.getChannelBookmarksRoute(channelId)}`, {
			method: 'post',
			body: JSON.stringify(channelBookmark),
	...
	getRemoteCluster = (remoteId: string) => {
		return this.doFetch<RemoteCluster>(`${this.getRemoteClusterRoute(remoteId)}`, {
			method: 'get',
		});
	};

	createRemoteCluster = (
		remoteCluster: PartialExcept<RemoteClusterWithPassword, 'name' | 'display_name'>,
	) => {
	...
	acceptInviteRemoteCluster = (remoteClusterAcceptInvite: RemoteClusterAcceptInvite) => {
		return this.doFetch<RemoteCluster>(`${this.getRemoteClustersRoute()}/accept_invite`, {
			method: 'post',
			body: JSON.stringify(remoteClusterAcceptInvite),
		});
	};

	generateInviteRemoteCluster = (
		remoteId: string,
		remoteCluster: Partial<Pick<RemoteClusterWithPassword, 'password'>>,
	) => {
		return this.doFetch<string>(`${this.getRemoteClusterRoute(remoteId)}/generate_invite`, {
			method: 'post',
			body: JSON.stringify(remoteCluster),
		});
	};
	...
	getCustomProfileAttributeFields = async () => {
		return this.doFetch<UserPropertyField[]>(`${this.getCustomProfileAttributeFieldsRoute()}`, {
			method: 'get',
		});
	};
	...
	patchCustomProfileAttributeField = async (fieldId: string, patch: UserPropertyFieldPatch) => {
		return this.doFetch<UserPropertyField>(`${this.getCustomProfileAttributeFieldRoute(fieldId)}`, {
			method: 'patch',
			body: JSON.stringify(patch),
		});
	};
	...
	deleteCustomProfileAttributeField = async (fieldId: string) => {
		return this.doFetch<StatusOK>(`${this.getCustomProfileAttributeFieldRoute(fieldId)}`, {
			method: 'delete',
		});
	};
	...
	updateUserCustomProfileAttributesValues = (
		userID: string,
	...
	createPostEphemeral = async (
		userID: string,
		post: PartialExcept<Post, 'channel_id' | 'message'>,
	) => {
		const result = await this.doFetch<Post>(`${this.getPostsRoute()}/ephemeral`, {
			method: 'post',
	...
	revealBurnOnReadPost = (postId: string) => {
		return this.doFetch<Post>(`${this.getPostRoute(postId)}/reveal`, { method: 'get' });
	};

	burnPostNow = (postId: string) => {
		return this.doFetch(`${this.getPostRoute(postId)}/burn`, { method: 'delete' });
	};
	...
	getPostThread = (
		...
		return this.getPaginatedPostThread(postId, {
			fetchThreads,
			collapsedThreads,
	...
	markThreadAsUnreadForUser = (
		...
		return this.doFetch<UserThread>(url, { method: 'post' });
	};
	...
	addPostReminder = (userId: string, postId: string, timestamp: number) => {
		return this.doFetch<StatusOK>(`${this.getUserRoute(userId)}/posts/${postId}/reminder`, {
			method: 'post',
			body: JSON.stringify({ target_time: timestamp }),
		});
	};
	...
	getPostInfo = (postId: string) => {
		return this.doFetch<PostInfo>(`${this.getPostRoute(postId)}/info`, { method: 'get' });
	};

	getPostsByIds = (postIds: string[]) => {
		return this.doFetch<Post[]>(`${this.getPostsRoute()}/ids`, {
			method: 'post',
	...
	getPostEditHistory = (postId: string) => {
		return this.doFetch<Post[]>(`${this.getPostRoute(postId)}/edit_history`, { method: 'get' });
	};
	...
	searchPostsWithParams = (teamId: string, params: any) => {
		...
		return this.doFetch<PostSearchResults>(route, { method: 'post', body: JSON.stringify(params) });
	};
	...
	searchFilesWithParams = (teamId: string, params: any) => {
		...
		return this.doFetch<FileSearchResults>(route, { method: 'post', body: JSON.stringify(params) });
	};
	...
	acknowledgePost = (postId: string, userId: string) => {
		return this.doFetch<PostAcknowledgement>(`${this.getUserRoute(userId)}/posts/${postId}/ack`, {
			method: 'post',
		});
	};

	unacknowledgePost = (postId: string, userId: string) => {
		return this.doFetch<null>(`${this.getUserRoute(userId)}/posts/${postId}/ack`, {
			method: 'delete',
		});
	};
	...
	getUserPreferences = (userId: string) => {
		return this.doFetch<PreferenceType[]>(`${this.getPreferencesRoute(userId)}`, { method: 'get' });
	};
	...
	isAllowedToUpgradeToEnterprise = async () => {
		return this.doFetch<StatusOK>(`${this.getBaseRoute()}/upgrade_to_enterprise/allowed`, {
			method: 'get',
		});
	};
	...
	getLicenseLoadMetric = () => {
		return this.doFetch<{
			load: number;
		}>(`${this.getBaseRoute()}/license/load_metric`, { method: 'get' });
	};
	...
	getFirstAdminVisitMarketplaceStatus = async () => {
		return this.doFetch<SystemSetting>(`${this.getPluginsRoute()}/marketplace/first_admin_visit`, {
			method: 'get',
		});
	};

	getFirstAdminSetupComplete = async () => {
		return this.doFetch<SystemSetting>(`${this.getSystemRoute()}/onboarding/complete`, {
			method: 'get',
		});
	};
	...
	createOAuthApp = (
		app: PartialExcept<OAuthApp, 'name' | 'description' | 'callback_urls' | 'homepage'>,
	) => {
	...
	getAppsOAuthAppIDs = () => {
		return this.doFetch<string[]>(`${this.getAppsProxyRoute()}/api/v1/oauth-app-ids`, {
			method: 'get',
		});
	};
	...
	getOutgoingOAuthConnectionsForAudience = (
		teamId: string,
		audience: string,
		page = 0,
		perPage = PER_PAGE_DEFAULT,
	...
	deleteOutgoingOAuthConnection = (connectionId: string) => {
		return this.doFetch<StatusOK>(`${this.getOutgoingOAuthConnectionRoute(connectionId)}`, {
			method: 'delete',
		});
	};
	...
	createCustomEmoji = (
		emoji: PartialExcept<CustomEmoji, 'name' | 'creator_id'>,
		imageData: File,
	) => {
		const formData = new FormData();
		formData.append('image', imageData);
	...
	getCustomEmojisByNames = (names: string[]) => {
		return this.doFetch<CustomEmoji[]>(`${this.getEmojisRoute()}/names`, {
			method: 'post',
	...
	createRecap = (request: CreateRecapRequest) => {
		return this.doFetch<Recap>(`${this.getRecapsRoute()}`, {
			method: 'post',
	...
	getRecap = (recapId: string) => {
		return this.doFetch<Recap>(`${this.getRecapsRoute()}/${recapId}`, { method: 'get' });
	};

	markRecapAsRead = (recapId: string) => {
		return this.doFetch<Recap>(`${this.getRecapsRoute()}/${recapId}/read`, { method: 'post' });
	};

	regenerateRecap = (recapId: string) => {
		return this.doFetch<Recap>(`${this.getRecapsRoute()}/${recapId}/regenerate`, {
			method: 'post',
		});
	};

	deleteRecap = (recapId: string) => {
		return this.doFetch<StatusOK>(`${this.getRecapsRoute()}/${recapId}`, { method: 'delete' });
	};

	// Admin Routes

	getLogs = (logFilter: LogFilterQuery) => {
		return this.doFetch<string[]>(`${this.getBaseRoute()}/logs/query`, {
			method: 'post',
	...
	patchConfig = (patch: DeepPartial<AdminConfig>) => {
		return this.doFetch<AdminConfig>(`${this.getBaseRoute()}/config/patch`, {
			method: 'put',
			body: JSON.stringify(patch),
		});
	};
	...
	// Agent Routes
	getAgents = () => {
		return this.doFetch<Agent[]>(`${this.getAgentsRoute()}`, { method: 'get' });
	};
	...
	getLLMServices = () => {
		return this.doFetch<LLMService[]>(`${this.getBaseRoute()}/llmservices`, { method: 'get' });
	};
	...
	sendTestNotificaiton = () => {
		return this.doFetch<StatusOK>(`${this.getBaseRoute()}/notifications/test`, { method: 'post' });
	};
	...
	testLdapConnection = (settings: LdapSettings) => {
		return this.doFetch<StatusOK>(`${this.getBaseRoute()}/ldap/test_connection`, {
			method: 'post',
	...
	testLdapDiagnostics = (settings: LdapSettings, testType: LdapDiagnosticTestType) => {
		return this.doFetch<StatusOK>(`${this.getBaseRoute()}/ldap/test_diagnostics?test=${testType}`, {
			method: 'post',
			body: JSON.stringify(settings),
		});
	};
	...
	uploadAuditLogCertificate = (fileData: File) => {
		...
		return this.doFetch<StatusOK>(`${this.getBaseRoute()}/audit_logs/certificate`, {
			method: 'post',
			body: formData,
		});
	};
	...
	requestTrialLicense = (body: RequestLicenseBody) => {
		return this.doFetchWithResponse<ClientLicense>(`${this.getBaseRoute()}/trial-license`, {
			method: 'post',
			body: JSON.stringify(body),
		});
	};
	...
	getPrevTrialLicense = () => {
		return this.doFetch<ClientLicense>(`${this.getBaseRoute()}/trial-license/prev`, {
			method: 'get',
		});
	};
	...
	getGroupStats = (groupID: string) => {
		return this.doFetch<GroupStats>(`${this.getGroupRoute(groupID)}/stats`, { method: 'get' });
	};

	getGroups = (opts: GetGroupsForUserParams | GetGroupsParams) => {
		return this.doFetch<Group[]>(`${this.getGroupsRoute()}${buildQueryString(opts)}`, {
			method: 'get',
		});
	};
	...
	getGroupsByNames = (names: string[]) => {
		return this.doFetch<Group[]>(`${this.getGroupsRoute()}/names`, {
			method: 'post',
			body: JSON.stringify(names),
		});
	};
	...
	createGroupWithUserIds = (group: GroupCreateWithUserIds) => {
		return this.doFetch<Group>(this.getGroupsRoute(), {
			method: 'post',
	...
	addUsersToGroup = (groupId: string, userIds: string[]) => {
		return this.doFetch<GroupMember[]>(`${this.getGroupRoute(groupId)}/members`, {
			method: 'post',
	...
	searchGroups = (params: GroupSearchParams) => {
		return this.doFetch<Group[]>(`${this.getGroupsRoute()}${buildQueryString(params)}`, {
			method: 'get',
		});
	};
	...
	restoreGroup = (groupId: string) => {
		return this.doFetch<Group>(`${this.getGroupRoute(groupId)}/restore`, { method: 'post' });
	};
	...
	cwsAvailabilityCheck = () => {
		return this.doFetch<{ status: string }>(`${this.getCloudRoute()}/check-cws-connection`, {
			method: 'get',
		});
	};
	...
	notifyAdmin = (req: NotifyAdminRequest) => {
		return this.doFetchWithResponse<StatusOK>(`${this.getUsersRoute()}/notify-admin`, {
			method: 'post',
	...
	getInstallation = () => {
		return this.doFetch<Installation>(`${this.getCloudRoute()}/installation`, { method: 'get' });
	};

	getCloudPreviewModalData = () => {
		return this.doFetch<PreviewModalContentData[]>(`${this.getCloudRoute()}/preview/modal_data`, {
			method: 'get',
		});
	};
	...
	getCloudLimits = () => {
		return this.doFetch<Limits>(`${this.getCloudRoute()}/limits`, { method: 'get' });
	};

	getPostsUsage = () => {
		return this.doFetch<PostsUsageResponse>(`${this.getUsageRoute()}/posts`, { method: 'get' });
	};

	getFilesUsage = () => {
		return this.doFetch<FilesUsageResponse>(`${this.getUsageRoute()}/storage`, { method: 'get' });
	};

	getTeamsUsage = () => {
		return this.doFetch<TeamsUsageResponse>(`${this.getUsageRoute()}/teams`, { method: 'get' });
	};
	...
	getAncillaryPermissions = (subsectionPermissions: string[]) => {
		return this.doFetch<string[]>(`${this.getPermissionsRoute()}/ancillary`, {
			method: 'post',
			body: JSON.stringify(subsectionPermissions),
		});
	};
	...
	getAppliedSchemaMigrations = () => {
		return this.doFetch<SchemaMigration[]>(`${this.getSystemRoute()}/schema/version`, {
			method: 'get',
		});
	};
	...
	getBookingAddresses = () => {
		return this.doFetch<BookingAddress[]>(this.getBookingAddressesRoute(), { method: 'get' });
	};

	getBookingAddress = (addressId: string) => {
		return this.doFetch<BookingAddress>(this.getBookingAddressRoute(addressId), {
			method: 'get',
		});
	};

	createBookingAddress = (address: BookingAddress) => {
		return this.doFetch<BookingAddress>(this.getBookingAddressesRoute(), {
			method: 'post',
			body: JSON.stringify(address),
		});
	};

	updateBookingAddress = (addressId: string, address: BookingAddress) => {
		return this.doFetch<BookingAddress>(this.getBookingAddressRoute(addressId), {
			method: 'put',
			body: JSON.stringify(address),
		});
	...
	deleteBookingAddress = (addressId: string) => {
		return this.doFetch<StatusOK>(this.getBookingAddressRoute(addressId), { method: 'delete' });
	};

	getBookingRoutes = () => {
		return this.doFetch<BookingRoute[]>(this.getBookingRoutesRoute(), { method: 'get' });
	};

	getBookingRouteById = (routeId: string) => {
		return this.doFetch<BookingRoute>(this.getBookingRouteByIdRoute(routeId), {
			method: 'get',
		});
	};

	createBookingRoute = (route: BookingRoute) => {
		return this.doFetch<BookingRoute>(this.getBookingRoutesRoute(), {
			method: 'post',
			body: JSON.stringify(route),
		});
	};

	updateBookingRoute = (routeId: string, route: BookingRoute) => {
		return this.doFetch<BookingRoute>(this.getBookingRouteByIdRoute(routeId), {
			method: 'put',
			body: JSON.stringify(route),
		});
	...
	deleteBookingRoute = (routeId: string) => {
		return this.doFetch<StatusOK>(this.getBookingRouteByIdRoute(routeId), { method: 'delete' });
	};

	getBookingTransportBrands = (opts: SearchOpts) => {
	...
	getBookingTransportBrand = (brandId: string) => {
		return this.doFetch<BookingTransportBrand>(this.getBookingTransportBrandRoute(brandId), {
			method: 'get',
		});
	};
	...
	updateBookingTransportBrand = (brandId: string, brand: BookingTransportBrand) => {
		return this.doFetch<BookingTransportBrand>(this.getBookingTransportBrandRoute(brandId), {
			method: 'put',
			body: JSON.stringify(brand),
		});
	...
	deleteBookingTransportBrand = (brandId: string) => {
		return this.doFetch<StatusOK>(this.getBookingTransportBrandRoute(brandId), {
			method: 'delete',
		});
	};

	getBookingVehicles = () => {
		return this.doFetch<BookingVehicle[]>(this.getBookingVehiclesRoute(), { method: 'get' });
	};

	getBookingVehicle = (vehicleId: string) => {
		return this.doFetch<BookingVehicle>(this.getBookingVehicleRoute(vehicleId), {
			method: 'get',
		});
	};

	createBookingVehicle = (vehicle: BookingVehicle) => {
		return this.doFetch<BookingVehicle>(this.getBookingVehiclesRoute(), {
			method: 'post',
			body: JSON.stringify(vehicle),
		});
	};

	updateBookingVehicle = (vehicleId: string, vehicle: BookingVehicle) => {
		return this.doFetch<BookingVehicle>(this.getBookingVehicleRoute(vehicleId), {
			method: 'put',
			body: JSON.stringify(vehicle),
		});
	...
	deleteBookingVehicle = (vehicleId: string) => {
		return this.doFetch<StatusOK>(this.getBookingVehicleRoute(vehicleId), { method: 'delete' });
	};

	getBookingSeats = () => {
		return this.doFetch<BookingSeat[]>(this.getBookingSeatsRoute(), { method: 'get' });
	};

	getBookingSeat = (seatId: string) => {
		return this.doFetch<BookingSeat>(this.getBookingSeatRoute(seatId), { method: 'get' });
	};

	createBookingSeat = (seat: BookingSeat) => {
		return this.doFetch<BookingSeat>(this.getBookingSeatsRoute(), {
			method: 'post',
			body: JSON.stringify(seat),
		});
	};

	updateBookingSeat = (seatId: string, seat: BookingSeat) => {
		return this.doFetch<BookingSeat>(this.getBookingSeatRoute(seatId), {
			method: 'put',
			body: JSON.stringify(seat),
		});
	};

	deleteBookingSeat = (seatId: string) => {
		return this.doFetch<StatusOK>(this.getBookingSeatRoute(seatId), { method: 'delete' });
	};

	getBookingSeatsByVehicle = (vehicleId: string) => {
		return this.doFetch<BookingSeat[]>(this.getBookingVehicleSeatsRoute(vehicleId), {
			method: 'get',
		});
	};

	getBookingTrips = () => {
		return this.doFetch<BookingTrip[]>(this.getBookingTripsRoute(), { method: 'get' });
	};

	getBookingTrip = (tripId: string) => {
		return this.doFetch<BookingTrip>(this.getBookingTripRoute(tripId), { method: 'get' });
	};

	createBookingTrip = (trip: BookingTrip) => {
		return this.doFetch<BookingTrip>(this.getBookingTripsRoute(), {
			method: 'post',
			body: JSON.stringify(trip),
		});
	};

	updateBookingTrip = (tripId: string, trip: BookingTrip) => {
		return this.doFetch<BookingTrip>(this.getBookingTripRoute(tripId), {
			method: 'put',
			body: JSON.stringify(trip),
		});
	};

	startBookingTrip = (tripId: string) => {
		return this.doFetch<StatusOK>(`${this.getBookingTripRoute(tripId)}/start`, { method: 'post' });
	};

	completeBookingTrip = (tripId: string) => {
		return this.doFetch<StatusOK>(`${this.getBookingTripRoute(tripId)}/complete`, {
			method: 'post',
		});
	};

	failBookingTrip = (tripId: string, request: FailBookingTripRequest) => {
		return this.doFetch<StatusOK>(`${this.getBookingTripRoute(tripId)}/fail`, {
			method: 'post',
			body: JSON.stringify(request),
		});
	};

	getBookingTripsByRoute = (routeId: string) => {
		return this.doFetch<BookingTrip[]>(this.getBookingTripsByRouteRoute(routeId), {
			method: 'get',
		});
	};

	getBookingTripsByVehicle = (vehicleId: string) => {
		return this.doFetch<BookingTrip[]>(this.getBookingTripsByVehicleRoute(vehicleId), {
			method: 'get',
		});
	};

	getBookingReservations = () => {
		return this.doFetch<BookingReservation[]>(this.getBookingReservationsRoute(), {
			method: 'get',
		});
	};

	getBookingReservation = (reservationId: string) => {
		return this.doFetch<BookingReservation>(this.getBookingReservationRoute(reservationId), {
			method: 'get',
		});
	};

	createBookingReservation = (reservation: BookingReservation) => {
		return this.doFetch<BookingReservation>(this.getBookingReservationsRoute(), {
			method: 'post',
	...
	updateBookingReservation = (reservationId: string, reservation: BookingReservation) => {
		return this.doFetch<BookingReservation>(this.getBookingReservationRoute(reservationId), {
			method: 'put',
			body: JSON.stringify(reservation),
		});
	...
	cancelBookingReservation = (reservationId: string, request: CancelBookingReservationRequest) => {
		return this.doFetch<StatusOK>(`${this.getBookingReservationRoute(reservationId)}/cancel`, {
			method: 'post',
			body: JSON.stringify(request),
		});
	};

	confirmBookingReservationPlacement = (reservationId: string) => {
		return this.doFetch<StatusOK>(
			`${this.getBookingReservationRoute(reservationId)}/confirm-placement`,
			{ method: 'post' },
		);
	};

	confirmBookingTripFinish = (reservationId: string) => {
		return this.doFetch<StatusOK>(
			`${this.getBookingReservationRoute(reservationId)}/confirm-trip-finish`,
			{ method: 'post' },
		);
	};

	getBookingReservationsByTrip = (tripId: string) => {
		return this.doFetch<BookingReservation[]>(this.getBookingReservationsByTripRoute(tripId), {
			method: 'get',
		});
	};

	getBookingReservationsByUser = (userId: string) => {
		return this.doFetch<BookingReservation[]>(this.getBookingReservationsByUserRoute(userId), {
			method: 'get',
		});
	};
	...
	protected doFetch = async <ClientDataResponse>(
		url: string,
		options: Options,
	...
	private doFetchWithResponse = async <ClientDataResponse>(
		url: string,
		options: Options,
	...
	upsertDraft = async (draft: Draft, connectionId: string) => {
		const result = await this.doFetch<Draft>(`${this.getDraftsRoute()}`, {
			method: 'post',
	...
	getUserDrafts = (teamId: Team['id']) => {
		return this.doFetch<Draft[]>(`${this.getUserRoute('me')}/teams/${teamId}/drafts`, {
			method: 'get',
		});
	};
	...
	deleteDraft = (channelId: Channel['id'], rootId = '', connectionId: string) => {
		...
		return this.doFetch<null>(endpoint, {
			method: 'delete',
			headers: {
				'Connection-Id': `${connectionId}`,
			},
	...
	getIPFilters = () => {
		return this.doFetch<AllowedIPRange[]>(`${this.getBaseRoute()}/ip_filtering`, { method: 'get' });
	};

	getCurrentIP = () => {
		return this.doFetch<FetchIPResponse>(`${this.getBaseRoute()}/ip_filtering/my_ip`, {
			method: 'get',
		});
	};

	applyIPFilters = (filters: AllowedIPRanges) => {
		return this.doFetch<AllowedIPRange[]>(`${this.getBaseRoute()}/ip_filtering`, {
			method: 'post',
	...
	getGroupMessageMembersCommonTeams = (channelId: string) => {
		return this.doFetchWithResponse<Team[]>(`${this.getChannelRoute(channelId)}/common_teams`, {
			method: 'get',
		});
	};
	...
	createScheduledPost = (
		schedulePost: PartialExcept<ScheduledPost, 'channel_id' | 'message' | 'scheduled_at'>,
		connectionId: string,
	) => {
		return this.doFetchWithResponse<ScheduledPost>(`${this.getPostsRoute()}/schedule`, {
			method: 'post',
	...
	getAccessControlPolicy = (id: string, channelId?: string) => {
		...
		return this.doFetch<AccessControlPolicy>(url, { method: 'get' });
	};

	updateOrCreateAccessControlPolicy = (policy: AccessControlPolicy) => {
		return this.doFetch<AccessControlPolicy>(`${this.getBaseRoute()}/access_control_policies`, {
			method: 'put',
			body: JSON.stringify(policy),
		});
	};
	...
	getChildPolicies = (parentId: string, after: string, limit: number) => {
		return this.doFetch<AccessControlPoliciesResult>(
			...
			{
				method: 'post',
				body: JSON.stringify({ parent_id: parentId, cursor: { id: after }, limit }),
			},
		);
	};
	...
	searchAccessControlPolicies = (term: string, type: string, after: string, limit: number) => {
		return this.doFetch<AccessControlPoliciesResult>(
			...
			{
				method: 'post',
				body: JSON.stringify({ term, type, cursor: { id: after }, limit, include_children: true }),
			},
		);
	};
	...
	flagPost = (postId: string, reason: string, comment?: string) => {
		return this.doFetch<StatusOK>(`${this.getContentFlaggingRoute()}/post/${postId}/flag`, {
			method: 'post',
	...
	removeFlaggedPost = (postId: string, comment?: string) => {
		return this.doFetch<StatusOK>(`${this.getContentFlaggingRoute()}/post/${postId}/remove`, {
			method: 'put',
	...
	keepFlaggedPost = (postId: string, comment?: string) => {
		return this.doFetch<StatusOK>(`${this.getContentFlaggingRoute()}/post/${postId}/keep`, {
			method: 'put',
			body: JSON.stringify({ comment: JSON.stringify(comment) }),
		});
	};
	...
	getPostContentFlaggingFields = () => {
		return this.doFetch<NameMappedPropertyFields>(`${this.getContentFlaggingRoute()}/fields`, {
			method: 'get',
		});
	};
	...
	getFlaggedPost = (postId: string) => {
		return this.doFetch<Post>(`${this.getContentFlaggingRoute()}/post/${postId}`, {
			method: 'get',
		});
	};
	...
	setContentFlaggingReviewer = (postId: string, reviewerId: string) => {
		return this.doFetch(`${this.getContentFlaggingRoute()}/post/${postId}/assign/${reviewerId}`, {
			method: 'post',
		});
	};

	saveContentFlaggingConfig = (config: ContentFlaggingSettings) => {
		return this.doFetch(`${this.getContentFlaggingRoute()}/config`, {
			method: 'put',
			body: JSON.stringify(config),
		});
	};
	...
	getAdminContentFlaggingConfig = () => {
		return this.doFetch<ContentFlaggingSettings>(`${this.getContentFlaggingRoute()}/config`, {
			method: 'get',
		});
	};
}
