export default {
	signin: {
		title: '로그인',
		signinButton: '로그인',
		rememberMe: '로그인 상태 유지',
		noAccount: '계정이 없으신가요?',
		forgotPassword: '비밀번호를 잊으셨나요?',
		welcomeBack: '다시 오신 것을 환영합니다, {{name}}님',
	},
	signup: {
		title: '회원가입',
		signupButton: '계정 생성',
		confirmPasswordPlaceholder: '비밀번호를 확인해주세요',
		agreeToTerms: '이용 약관에 동의합니다',
		alreadyHasAccount: '이미 계정이 있으신가요?',
		signupSuccess:
			'Welcome to Sitename. An email is sent to your email address. Please check your inbox.',
		promtGeoAccessPerm:
			'We need your precise location to provide better experience. Please accept location request. Thank you!',
	},
	resetPassword: {
		title: '비밀번호 재설정',
		btnText: '이메일 보내기',
		emailSent: '이메일을 보내려습니다. 홈보드에서 확인해주세요',
	},
	changePassword: {
		title: '비밀번호 변경',
		btnText: '비밀번호 변경',
		invalidUrl: 'URL을 확인해주세요',
	},
	common: {
		home: '홈',
		emailPlaceholder: '이메일을 입력해주세요',
		yes: '예',
		no: '아니오',
		passwordPlaceholder: '비밀번호를 입력해주세요',
		level1: '레벨 1',
		level2: '레벨 2',
		level3: '레벨 3',
		level4: '레벨 4',
		level5: '레벨 5',
		level6: '레벨 6',
		more: '더 보기',
		settings: '설정',
		logout: '로그아웃',
		loadMore: '더 불러오기',
		categories: '카테고리',
		ordering: '정렬 기준',
		priceRange: '가격 범위',
		score: '평점',
		filter: '필터',
		selectFilter: 'Select filter',
		delFilter: 'Delete filter',
		addFilter: 'Add filter',
		reset: 'Reset',
		star: '별',
		cart: '장바구니',
		startAt: '시작',
		discountRange: '할인 범위',
		packaging: '포장',
		preorder: '사전 주문',
		ok: '확인',
		cancel: '취소',
		confirmRemoveProduct: '장바구니에서 이 제품을 삭제하시겠습니까?',
		city: '도시',
		firstName: '이름',
		lastName: '성',
		countryArea: '국가 지역',
		country: '국가',
		cityArea: '도시 지역',
		postalCode: '우편 번호',
		companyName: '회사 이름',
		streetAddress1: '주소 1',
		streetAddress2: '주소 2',
		phone: '전화 번호',
		action: '작동',
		rating: '평점',
		name: '이름',
		publicationDate: '출판일',
		minimalPrice: '최소 가격',
		price: '가격',
		advanced: '연급',
		email: '이메일',
		password: '비밀번호',
		confirmPwd: '비밀번호 확인',
		from: 'From',
		to: 'To',
		prevPage: 'Previous page',
		nextPage: 'Next page',
		pic: 'Picture',
		currency: 'Currency',
		generalInfo: 'General information',
	},
	product: {
		chooseAddress: '주소를 선택해주세요',
		delivery: '배송지',
		quantity: '수량',
		variants: '변형',
		addToCart: '장바구니에 추가',
		prdInformation: '제품 정보',
		tabDescription: '설명',
		tabFeedBack: '고객 피드백',
		tabAttributes: '속성',
		tabPackaging: '포장',
		variantAvailable: '{{quantity}} 개 이용 가능',
		prdName: '제품 이름',
		prdCategory: '제품 카테고리',
		prdDescription: '제품 설명',
		prdVariant: '제품 변형',
		variant: '변형',
		variantName: '이름',
		delVariant: '변형 삭제',
		delValue: '값 삭제',
		addVariant: '변형 추가',
		addValue: '값 추가',
		variantNameEmpty: '변형 이름은 비워둘 수 없습니다',
		variantValueEmpty: '변형 값은 비워둘 수 없습니다',
		variantNameExist: "변형 '{{name}}'은 이미 존재합니다",
		variantValueExist: "값 '{{name}}'은 이미 존재합니다",
		variantPlaceholder: '변형 이름을 입력해주세요',
		category: '카테고리',
		products: '제품',
		subCategories: '하위 카테고리',
		detail: '세부사항',
		noVote: '아직 투표가 없습니다',
		noDescription: '아직 설명이 없습니다',
		prdPolicy: '제품 정책',
		prdPolicyDetail:
			'Sitename guarantees provide high quality product to our customers. Sitename refunds 100% if the product does not meet quality standard.',
		quickFilling: '쓰기',
		price: '가격',
		costPrice: '입고 가격',
		channel: '커널',
		stock: '수량',
		sku: 'sku',
		fromPrd: 'từ sản phẩm',
		toPrd: 'tới sản phẩm',
		addDiscountRange: 'thêm khoảng giảm giá',
		weight: 'weight',
		length: 'Length',
		width: 'Width',
		height: 'Height',
		noAttributes: 'No attributes found',
		prdSlug: 'product slug',
		seo: 'product SEO',
		seoTitle: 'product SEO title',
		seoDescription: 'product SEO description',
		channelHint: 'Channels are where your products will be published.',
		priceHint: 'Price is selling price for each variant.',
		costPriceHint: 'Cost Price is cost price for each variant.',
		stockHint: 'Stock is quantity of each variant in specific warehouse.',
		weightHint: 'Weight is weight of each variant in kg.',
		skuHint: 'Stock Keeping Unit of each variant, uniquely generated by default.',
		qtyLimit: 'hạn mức',
		preOrderEndDate: 'hạn giao hàng',
		trackInventory: 'track inventory',
		collection: 'Bộ sưu tập',
		taxCls: 'Lớp thuế',
		prdType: 'Loại sản phẩm',
		published: 'Published',
		promptPublicationTime: 'If not, please specify publidation time',
		availForPurchase: 'Available for purchase',
		promptAvailTime: 'If not, please specify availability time',
		preorderHint:
			'Pre-order is when you must order the product from other suppliers. The days it takes to delivery must be between {{min}} to {{max}} working days.',
		prdCreated: 'Product created successfully',
		prdUpdated: 'Product updated successfully',
	},
	error: {
		invalidSlug: '잘못된 슬러그 값',
		invalidEmailAndPassword: '유효한 이메일과 비밀번호를 입력해주세요',
		invalidEmail: '유효한 이메일을 입력해주세요',
		passwordsNotMatch: '비밀번호가 일치하지 않습니다',
		noVariantSelected: '변형을 선택해주세요',
		negativeNumber: '양수를 입력해주세요',
		startGreaterEnd: '시작은 끝보다 작아야 합니다',
		noResult: '결과를 찾을 수 없습니다. 다시 시도해주세요',
		failedToLoad: '데이터를 불러오지 못했습니다. 나중에 다시 시도해주세요',
		errorOccured: '로그아웃에 실패했습니다. 나중에 다시 시도해주세요',
		positiveInteger: '양수를 입력해주세요',
		lengthInvalid: '{{name}} must have from {{min}} to {{max}} characters.',
		initEditorErr: 'Failed to initialize editor. Please try again later',
		thereIsError: 'There are some error(s). Please fix them to proceed.',
		doYouAgree: 'Are you agree with our term and policies?',
		itemsExceed: 'At most {{max}} items allowed',
	},
	placeholders: {
		enterPrdName: '제품 이름을 입력해주세요',
		valuePlaceholder: '값을 입력해주세요',
	},
	pages: {
		cart: '장바구니',
		checkout: '결제',
		payment: '지불',
		orderConfirm: '주문 확인',
		home: '홈',
		trending: '픽리텍',
	},
	btn: {
		apply: '조회',
		create: '타입 생성',
		update: '타입 변경',
		delete: 'Delete',
		goto: 'Đi tới',
		back: 'Back',
		add: 'Add',
	},
	helpText: {
		chooseDeliveryAddress: '배송지를 선택해주세요',
		selectDate: '날짜를 선택해주세요',
		fieldRequired: '이 필드는 필요합니다',
		noData: 'There is no data',
	},
	footer: {
		socialMedia: 'Mạng xã hội',
		legal: 'Pháp lý',
		aboutUs: 'Về chúng tôi',
		language: 'Ngôn ngữ',
		policy: 'Điều khoản & chính sách',
		blog: 'Blog',
	},
	cart: {
		proceedCheckout: 'Proceed to checkout',
		continueShopping: 'Continue shopping',
		applyCode: 'Apply code',
		emptyCart: '장바구니에 제품이 없습니다',
		cartSummary: '장바구니 요약',
		tempoTotalPrice: '임시 총 가격',
		oldTotalPrice: '원래 총 가격',
		savings: '절약',
	},
	settings: {
		preview: 'Preview',
		verified: 'Verified',
		account: 'Account',
		preference: 'Preference',
		shopping: 'Shopping',
		myOrders: 'My Orders',
		supports: 'Supports',
		shopManage: 'Shop Management',
		contracts: 'Contracts',
		basicInfo: 'Basic Information',
		addrManage: 'Address Management',
		havingNoAddress: "You don't have any address",
		makeDefaultBillingAddr: 'Set as default billing address',
		makeDefaultShippingAddr: 'Set as default shipping address',
		newAddr: 'New Address',
		confirmDelAddr: 'Bạn có chắc chắn muốn xoá địa chỉ này?',
		pwdUpdate: 'Update Password',
		oldPwd: 'Old Password',
		newPwd: 'New Password',
		confirmPwd: 'Confirm Password',
		chooseCountry: 'Chọn quốc gia',
		useThisAddr: 'Use this address',
		addrDeleted: 'Address deleted successfully',
		addrUpdated: 'Address updated successfully',
		addrCreated: 'Address created successfully',
		accountUpdated: 'Tài khoản đã được cập nhật',
		passwordUpdated: 'Password updated successfully. Please login again with your new password',
		orders: 'Orders',
		newOrder: 'New Order',
		no: 'No',
		date: 'Date',
		payment: 'Payment',
		status: 'Status',
		total: 'Total',
		action: 'Action',
		reqSupport: 'Request support',
		requests: 'Yêu cầu',
		newRequest: 'Yêu cầu mới',
		title: 'Tiêu đề',
		tag: 'Tag',
		editTicket: 'Sửa yêu cầu',
		deleteTicket: 'Xoá yêu cầu',
		confirmDelRequest: 'Bạn có chắc chắn muốn xoá yêu cầu {{id}}?',
		consult: 'Consult',
		warranty: 'Warranty',
		description: 'Mô tả',
		attachments: 'Attachments',
		orderNo: 'Số Hiệu Đơn hàng',
		newPrd: 'Tạo sản phẩm mới',
		name: 'Name',
		availability: 'Availability',
		prices: 'Prices',
		createdAt: 'Created',
		configs: 'Configurations',
		staffs: 'Staffs',
		channelLinkHelpTxt: 'Define and manage your sales channels',
		staffLinkHelpTxt: 'Manage your employees and their permissions',
		users: 'Users',
	},
	payment: {
		clickAndCollectShippingMethod: 'click&collect',
		shippingDoesNotApply: 'does not apply',
		paymentTitle: 'Payment balance',
		subtotal: 'Subtotal',
		itemCount: '{{quantity}} items',
		taxes: 'Taxes',
		vatIncluded: 'VAT included',
		vatNotIncluded: 'does not apply',
		shipping: 'Shipping',
		shippingNotApplicable: 'does not apply',
		discount: 'Discount',
		total: 'Total',
		preauthorized: 'Preauthorized amount',
		captured: 'Captured amount',
		refunded: 'Refunded amount',
		outstanding: 'Outstanding Balance',
		includedInSubtotal: 'Included in subtotal',
		includedInPrices: 'Included in prices',
		settled: 'Settled',
		capture: 'Capture',
		refund: 'Refund',
		void: 'Refund',
		markAsPaid: 'Mark as paid',
	},
	staff: {
		staffs: 'Staffs',
		newStaff: 'New Staff',
		avatar: 'Avatar',
		active: 'Active',
		inactive: 'Inactive',
		fullName: 'Full Name',
		email: 'Email',
		status: 'Status',
		joinedSince: 'Joined since',
		staffCreated: 'Staff created successfully',
		staffUpdated: 'Staff updated successfully',
		staffDeleted: 'Staff deleted successfully',
		staffDeleteConfirm: 'Are you sure you want to delete the staff {{id}}?',
	},
	giftcard: {
		title: '기프트 카드',
		code: '코드',
		balance: '잔액',
		usedAmount: '사용 금액',
		initialBalance: '초기 잔액',
		currentBalance: '현재 잔액',
		expiryDate: '만료일',
		status: '상태',
		active: '활성',
		expired: '만료됨',
		deactivated: '비활성화됨',
		used: '사용됨',
		usedAt: '사용일',
		issueNew: '새 기프트 카드 발급',
		issueAt: '발급일',
		issuedBy: '발급자',
		creationDate: '생성일',
		edit: '기프트 카드 수정',
		delete: '기프트 카드 삭제',
		bulkActions: '일괄 작업',
		activate: '활성화',
		deactivate: '비활성화',
		confirmDelete: '선택한 기프트 카드를 삭제하시겠습니까?',
		confirmDeactivate: '선택한 기프트 카드를 비활성화하시겠습니까?',
		event: '기프트 카드 내역',
		noGiftcards: '기프트 카드를 찾을 수 없습니다',
		cardInfo: '카드 정보',
		resendCode: '코드 재전송',
		orderNumber: '주문번호',
		productBoughtToGetGiftCard: '상품 구매로 기프트 카드를 받음',
		filter: {
			searchPlaceholder: '기프트 카드 검색...',
			product: '상품',
			usedBy: '사용자',
			usedByStaff: '직원이 사용',
			usedByCustomer: '고객이 사용',
			usedByEmail: '이메일로 사용',
		},
		expiry: {
			labelExact: 'Exact date',
			placeholderExact: 'Select expiry date',
			labelIn: 'After',
			optionExact: 'Exact date',
			optionIn: 'After a period of time',
			willExpireOn: 'Will expire on:',
		},
		form: {
			channel: 'Channel',
			amount: 'Amount',
			unit: '단위',
			tags: '태그',
			customer: '고객',
			note: '메모',
			expiryDate: '만료일',
			expiryType: '만료 유형',
			save: '기프트 카드 저장',
			success: '기프트 카드가 성공적으로 저장되었습니다',
			error: '기프트 카드 저장 중 오류가 발생했습니다',
			requiredField: '필수 항목입니다',
			invalidAmount: '유효한 금액을 입력해주세요',
			channelPlaceholder: 'Please specify channel',
			customerPlaceholder: 'Please specify customer',
			tagsPlaceholder: 'Please specify tags',
			currencyPlaceholder: 'Please specify currency',
			notePlaceholder: 'Note for this giftcard',
			amountPlaceholder: 'Enter amount',
			unitPlaceholder: 'Enter unit',
			expiryTypePlaceholder: 'Select expiry type',
			setExpirationDate: 'Set expiration date',
			activeSubText: 'All issued cards require activation by staff before use.',
			noteSubText:
				'Why was this gift card issued. This note will not be shown to the customer. Note will be stored in gift card history',
			channelSubText: 'Customer will be sent the gift card code via this channels email address',
		},
		events: {
			date: '날짜',
			user: '사용자',
			order: '주문',
			balanceChange: '잔액 변동',
			noEvents: '이 기프트 카드에 대한 내역이 없습니다',
		},
		types: {
			ACTIVATED: 'Gift card was activated',
			BALANCE_RESET: 'Gift card balance was reset',
			BOUGHT: 'Gift card was bought',
			DEACTIVATED: 'Gift card was deactivated',
			EXPIRY_DATE_UPDATED: 'Gift card expiry date was updated',
			ISSUED: 'Gift card was issued',
			NOTE_ADDED: 'Gift card note was added',
			RESENT: 'Gift card was resent',
			SENT_TO_CUSTOMER: 'Gift card was sent to customer',
			TAGS_UPDATED: 'Gift card tags were updated',
			UPDATED: 'Gift card was updated',
			USED_IN_ORDER: 'Gift card was used in order',
		},
	},
	channel: {
		confirmDelChannel: 'Are you sure you want to delete the channel {{id}}?',
		edit: 'Edit channel',
		del: 'Delete channel',
		new: 'New channel',
		delSuccess: 'Successfully deleted channel',
		editSuccess: 'Successfully updated channel',
		newSuccess: 'Successfully created channel',
		chanToReplace: 'Channel to replace',
		replaceChanHelpTxt:
			'Specify a new channel to assign products to. The replace channel must have the same currency as deleting channel',
		channels: 'Channels',
		valueOutOfRange: 'This value must be >= {{min}} and <= {{max}}',
		orderExp: 'Order expiration',
		delExprOrdersSubTxt:
			'The time in days after expired orders will be deleted. Allowed range between {{min}} to {{max}}',
		allowUnpaidOrder: 'Allow unpaid orders',
		allowCompleteOrderBeforePay:
			'Enables completing checkout with order before a successful payment.`',
		useTranFlow: 'Use Transaction flow when marking order as paid',
	},
};
