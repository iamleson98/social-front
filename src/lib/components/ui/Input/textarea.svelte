	...
	<div class={`${INPUT_CLASSES[variant].fg}`}>
		<div class={`relative`}>
			...
			<textarea
				...
				class={`${rest.disabled ? 'text-gray-300! cursor-not-allowed!' : ''} w-full text-sm placeholder:opacity-55 rounded-lg ring-1 focus:ring-2 outline-none! field-sizing-content inline-block px-2.5 py-1 ${inputClass} ${INPUT_CLASSES[variant].bg} ${startIcon ? 'ps-8' : ''} ${TEXT_AREA_SIZE_MAP[size]}`}
				{...rest}
			></textarea>
