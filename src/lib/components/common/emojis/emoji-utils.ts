...
import { EmojiIndicesByUnicode, Emojis } from './emojis';
import type { Emoji, SystemEmoji } from './types';
...
const getEmojiName = (emoji: Emoji, searchedName: string) => {
	...
	if (searchedName && 'short_names' in emoji) {
		return (
			emoji.short_names.find((alias: string) => alias.startsWith(searchedName)) || emoji.short_name
		);
	}
...
export function convertEmojiSkinTone(emoji: SystemEmoji, newSkinTone: string): SystemEmoji {
	...
	if (currentSkinTone === 'default') {
		...
		// @ts-ignore
		const variation = Object.keys(emoji?.skin_variations).find((skinVariation) =>
			skinVariation.includes(newSkinTone),
		);
