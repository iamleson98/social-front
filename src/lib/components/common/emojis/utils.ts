...
import { compareEmojis, convertEmojiSkinTone, emojiMatchesSkin, getSkin } from './emoji-utils';
import { EmojiIndicesByCategory, Emojis } from './emojis';
import {
	CATEGORIES,
	CATEGORY_HEADER_ROW,
	EMOJI_PER_ROW,
...
export function getFilteredEmojis(
	...
	const filteredRecentEmojis = filteredEmojisWithRecent.filter((emoji) =>
		recentEmojis.some((item) => isEmojiIdEqual(item, emoji)),
	);
	...
	const filtertedEmojisMinusRecent = filteredEmojisWithRecent.filter(
		(emoji) => !recentEmojis.some((item) => isEmojiIdEqual(item, emoji)),
	);

	const sortedFiltertedEmojisMinusRecent = filtertedEmojisMinusRecent.sort(
		(firstEmoji, secondEmoji) => compareEmojis(firstEmoji, secondEmoji, filter),
	);

	return sortedRecentEmojis
		.concat(sortedFiltertedEmojisMinusRecent)
		.filter((emoji) => emojiMatchesSkin(emoji, userSkinTone));
}
...
function getEmojisByCategory(allEmojis: Record<string, Emoji>, category: Category): Emoji[] {
...
export function createCategoryAndEmojiRows(
	...
	Object.keys(categories).forEach((categoryName, categoryIndex) => {
		const emojis = getEmojisByCategory(allEmojis, categories[categoryName as EmojiCategory]);
...
export function getUpdatedCategoriesAndAllEmojis(
	...
	Object.keys(categories).forEach((categoryName) => {
		...
		if (categoryName === 'recent' && recentEmojis.length) {
			...
			const indices =
				(EmojiIndicesByCategory.get(userSkinTone) as Map<string, number[]>).get(categoryName) || [];
		...
		categories[categoryName as EmojiCategory].emojiIds = categoryEmojis.map((emoji: Emoji) =>
