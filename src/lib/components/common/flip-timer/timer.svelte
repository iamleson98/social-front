<script lang="ts">
	...
	const segmentsCount = $derived(Number(showHours) + Number(showMinutes) + Number(showSeconds));

	let interval = $state<ReturnType<typeof setInterval>>();
	let display = $state(
		new Array(Number(showHours) + Number(showMinutes) + Number(showSeconds)).fill({
			top: '00',
			bottom: '00',
			transition: false,
		}),
