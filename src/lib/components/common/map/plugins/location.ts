import type { DomUtil } from "../types";

/**
 * Returns an object for the parent to add to Control
 * 
 * L.Control.extend(...)
 */
export const createLocationControl = (domUtil: DomUtil) => {
  const result = {
    // button position
    options: {
      position: "topleft",
    ...
    onAdd: function (map: any) {
      this._map = map;
      const button = domUtil.create("div");
