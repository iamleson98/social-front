...
<div class="rounded-lg overflow-y-auto max-h-60 w-1/3 p-2">
	{#each items as item, idx (idx)}
		...
		<div
			class={`flex items-center select-none justify-between rounded-lg mb-2 font-medium p-2 ${active ? 'text-blue-700 bg-blue-50 ring-2 ring-blue-600' : 'ring-gray-200 ring-1 text-gray-700'}`}
			role="button"
			tabindex="0"
