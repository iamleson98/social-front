...
export class GiftcardUtil {
	async handleToggleGiftcardStatus(id: string, active: boolean) {
		const [query, resultKey] = active
			? [GIFT_CARD_ACTIVATE_MUTATION, 'giftCardActivate' as keyof Mutation]
			: [GIFT_CARD_DEACTIVATE_MUTATION, 'giftCardDeactivate' as keyof Mutation];
