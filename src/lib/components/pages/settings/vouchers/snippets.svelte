<script lang="ts" module>
        ...
        /**
         * NOTE: `child` values refer to the top-level `{#snippet}` declarations
         * below. The Svelte compiler resolves and binds them at build time, but
         * svelte-check's language server cannot see that binding — a known
         * tooling limitation — hence the `@ts-expect-error` guards.
         */
        export const COLLECTION_COLUMNS = (
                ...
                {
                        title: nameTitle,
                        // NOTE: no guard needed — TS resolves `name` to the DOM global, while the Svelte compiler correctly binds the snippet.
                        child: name,
                },
                {
