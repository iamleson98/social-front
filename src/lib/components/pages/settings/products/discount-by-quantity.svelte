...
<div class="mb-3">
	<span class="text-sm">{$T('common.discountRange')}</span>
	<div class="border rounded-lg bg-gray-50 border-gray-200 p-3">
		<div class="flex items-center flex-row gap-2 mb-2 text-xs text-gray-700 font-semibold">
			...
			<div class="w-10/12 flex items-center gap-1">
				<div class="w-3/12">{$T('product.channel')}</div>
				<div class="w-3/12">{$T('product.fromPrd')}</div>
				<div class="w-3/12">{$T('product.toPrd')}</div>
				<div class="w-3/12">{$T('product.price')}</div>
			</div>
			<div class="w-1/12">
				<div class="tooltip" data-tip={$T('product.addDiscountRange')}>
					<IconButton
						icon={Plus}
						size="xs"
