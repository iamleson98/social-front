...
import { getGraphqlApiEndPoint } from '$lib/utils/env';
 ...
 * It MUST stay readable by the client (httpOnly: false) because the
...
/**
 * Cookie options for the refresh + CSRF tokens.
 * These are ONLY ever read by SvelteKit server endpoints, so they are
...
export const GRAPHQL_CLIENT = new Client({
        // NOTE: resolved at runtime via $env/dynamic/public — a static import
        // would break builds in CI environments where the variable is absent.
        url: getGraphqlApiEndPoint(),
        exchanges: [
