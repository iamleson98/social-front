...
export const load: LayoutServerLoad = async (event) => {
        ...
        const productDetailResult = await performServerSideGraphqlRequest<
                Pick<Query, 'product'>,
                QueryProductArgs
        >(PRODUCT_DETAIL_QUERY, variables, event, { requestPolicy: 'cache-and-network' });
        ...
        const variants = product?.productVariants?.edges?.map((edge) => edge.node) ?? [];
        ...
        const productJsonLd: WithContext<Product> = {
                ...
                ...(selectedVariant
                        ...
                        : {
                                        ...
                                        offers: {
                                                '@type': 'AggregateOffer',
                                                availability: variants.some((variant) => variant.quantityAvailable)
                                                        ? 'https://schema.org/InStock'
                                                        : 'https://schema.org/OutOfStock',
