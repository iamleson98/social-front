...
import {
        authTokenCookieOpts,
        cookieOpts,
        performServerSideGraphqlRequest,
} from '$lib/api/client';
...
export const POST = async (event) => {
        ...
        if (!email.trim() || !password) {
                return json({
                        error: await serverSideTranslate(event, 'error.invalidEmailAndPassword'),
        ...
        const authTokenPublicCookieOpts = { ...cookieOpts, maxAge: rememberMe ? 60 * 60 * 24 : undefined };
        const authTokenPrivateCookieOpts = {
                ...authTokenCookieOpts,
                maxAge: rememberMe ? 14 * 24 * 60 * 60 : undefined,
        };

        event.cookies.set(
                ACCESS_TOKEN_KEY,
                result.data?.tokenCreate?.token as string,
                authTokenPublicCookieOpts,
        );
        event.cookies.set(
                CSRF_TOKEN_KEY,
                result.data?.tokenCreate?.csrfToken as string,
                authTokenPrivateCookieOpts,
        );
        event.cookies.set(
                REFRESH_TOKEN_KEY,
                result.data?.tokenCreate?.refreshToken as string,
                authTokenPrivateCookieOpts,
        );
