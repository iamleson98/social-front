<script lang="ts">
	...
	import { getLocalUrl } from '$lib/utils/env';
	...
	const handleRequestResetPassword = async () => {
		...
		>(USER_REQUEST_PASSWORD_RESET_MUTATION_STORE, {
			email,
			redirectUrl: getLocalUrl() + AppRoute.AUTH_CHANGE_PASSWORD(),
			channel: clientSideGetCookieOrDefault(CHANNEL_KEY, DEFAULT_CHANNEL.slug),
		});
		loading = false;

		checkIfGraphqlResultHasError(result, 'requestPasswordReset', $T('resetPassword.emailSent'));
	};
</script>
